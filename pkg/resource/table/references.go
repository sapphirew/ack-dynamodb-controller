...
func (rm *resourceManager) EnsureReferences(
	...
	if desiredKO.Spec.SSESpecification != nil {
		if desiredKO.Spec.SSESpecification.KMSMasterKeyRef != nil {
			if latestKO.Spec.SSESpecification == nil {
				latestKO.Spec.SSESpecification = &svcapitypes.SSESpecification{}
			}
			if latestKO.Spec.SSESpecification.KMSMasterKeyRef == nil {
				latestKO.Spec.SSESpecification.KMSMasterKeyRef = desiredKO.Spec.SSESpecification.KMSMasterKeyRef
			}
		}
	}
...
func (rm *resourceManager) ResolveReferences(
	...
	if fieldHasReferences, err := rm.resolveReferenceForSSESpecification_KMSMasterKeyID(ctx, apiReader, ko); err != nil {
...
// identifier field.
func validateReferenceFields(ko *svcapitypes.Table) error {
...
// resolveReferenceForSSESpecification_KMSMasterKeyID reads the resource referenced
// from SSESpecification.KMSMasterKeyRef field and sets the SSESpecification.KMSMasterKeyID
// from referenced resource. Returns a boolean indicating whether a reference
// contains references, or an error
func (rm *resourceManager) resolveReferenceForSSESpecification_KMSMasterKeyID(
	ctx context.Context,
	apiReader client.Reader,
	ko *svcapitypes.Table,
) (hasReferences bool, err error) {
	if ko.Spec.SSESpecification != nil {
		if ko.Spec.SSESpecification.KMSMasterKeyRef != nil && ko.Spec.SSESpecification.KMSMasterKeyRef.From != nil {
			hasReferences = true
			arr := ko.Spec.SSESpecification.KMSMasterKeyRef.From
			if arr.Name == nil || *arr.Name == "" {
				return hasReferences, fmt.Errorf("provided resource reference is nil or empty: SSESpecification.KMSMasterKeyRef")
			}
			...
			ko.Spec.SSESpecification.KMSMasterKeyID = (*string)(obj.Status.ACKResourceMetadata.ARN)
		}
	}
